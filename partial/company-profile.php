...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $yhendus->prepare("UPDATE company_users SET first_name=?, last_name=?,company_name=?, registration_code=?,address=?,telephone=?, email=? WHERE company_id=?");
    $stmt->bind_param("sssssisi", $first_name, $last_name, $company_name, $registration_code, $address, $telephone,$email, $company_id);
...
// Fetch user data
$sql = "SELECT * FROM company_users WHERE company_id  = ?";
                        ...
                        <div class="card p-2 mb-5">
                            ...
                            <form id="profileForm" method="post" action="">
                                <table class="table table-borderless">
                                    ...
                                    <tr class="table-row">
                                        <td>Firma Nimi:</td>
                                        <td><input type="text" name="company_name" value="<?= $user['company_name'] ?>" class="form-control" disabled></td>
                                    </tr>
                                    <tr class="table-row">
                                        <td>Registrikood:</td>
                                        <td><input type="text" name="registration_code" value="<?= $user['registration_code'] ?>" class="form-control" disabled></td>
                                    </tr>
                                    <tr class="table-row">
                                        <td>Address:</td>
                                        <td><input type="text" name="address" value="<?= $user['address'] ?>" class="form-control" disabled></td>
                                    </tr>
                                    <tr class="table-row">
                                        <td>Telephone:</td>
                                        <td><input type="text" name="telephone" value="<?= $user['telephone'] ?>" class="form-control" disabled></td>
                                    </tr>
...
<script>
    $(document).ready(function () {
        ...
        $("#profileForm").submit(function (event) {
            ...
            $.ajax({
                type: "POST",
                url: "company-profile.php", // PHP script to handle the update
                data: formData,
